...
import { ArrowRight, CheckCircle2, Award, X } from "lucide-react";
...
export default function SuccessStories() {
  ...
  return (
    <div className="flex-1 bg-brand-bg pt-20 md:pt-28 pb-16 min-h-screen font-sans">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-black mb-4 font-plus-jakarta uppercase leading-tight">
            Success Stories
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {stories.map((story) => (
            <div
              key={story.id}
              className="sharp-card p-8 flex flex-col justify-between h-full hover:border-brand-purple transition-all duration-300 group"
            >
              <div>
                <span className="text-brand-purple text-[10px] font-bold uppercase tracking-wider block mb-4 font-inter">
                  {story.client}
                </span>
                
                <h3 className="text-2xl font-bold text-black mb-3 font-plus-jakarta uppercase tracking-wide group-hover:text-brand-purple transition-colors">
                  {story.title}
                </h3>
                
                <div className="text-3xl font-black text-black my-4 font-plus-jakarta">
                  {story.metric}
                </div>

                <p className="text-xs md:text-sm text-gray-500 font-inter leading-relaxed mb-6">
                  {story.desc}
                </p>

                {/* Impact Grid */}
                <div className="grid grid-cols-3 gap-2 border-t border-brand-border-light pt-6 mt-6">
                  {story.impact.map((imp) => (
                    <div key={imp.label} className="text-center">
                      <div className="text-base font-bold text-black font-plus-jakarta">{imp.value}</div>
                      <div className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-1 font-inter">
                        {imp.label}
                      </div>
                    </div>
                  ))}
              ...
              <button
                ...
                className="w-full mt-8 bg-black hover:bg-brand-purple text-white font-bold py-3.5 rounded-none transition-all text-xs uppercase tracking-wider flex items-center justify-center gap-2 border border-black hover:border-brand-purple font-inter font-bold text-[10px] uppercase"
              >
      ...
      {selectedCase !== null && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 z-50 overflow-y-auto">
            <div className="bg-white border border-brand-border p-8 md:p-12 rounded-none max-w-2xl w-full relative my-10 max-h-[85vh] overflow-y-auto scrollbar-thin">
              <button
              ...
              <span className="text-brand-purple text-[10px] font-bold uppercase tracking-widest font-inter block mb-3">
                {story.client} • Case Analysis
              </span>
              <h3 className="text-2xl md:text-3xl font-bold text-black mb-6 font-plus-jakarta uppercase tracking-wide leading-tight">
                {story.title}
              </h3>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h4 className="text-sm font-bold uppercase text-black tracking-wider mb-2 font-plus-jakarta">
                    The Challenge
                  </h4>
                ...
                <div>
                  <h4 className="text-sm font-bold uppercase text-black tracking-wider mb-4 font-plus-jakarta">
                    Implemented Partner Actions
                  </h4>
                  <div className="space-y-3">
                    {story.details.actions.map((act) => (
                      <div key={act.area} className="flex gap-3 items-start bg-brand-bg border border-brand-border p-4 rounded-none">
                        <CheckCircle2 className="h-5 w-5 text-brand-purple shrink-0 mt-0.5" />
                        <div>
                          <strong className="text-brand-purple text-[10px] uppercase block mb-0.5 font-inter font-bold tracking-widest">
                            {act.area} Area
                          </strong>
                          <span className="text-xs md:text-sm text-gray-600 leading-relaxed font-inter">{act.act}</span>
                        </div>
                      </div>
                    ))}
                  </div>
